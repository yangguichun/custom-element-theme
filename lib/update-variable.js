...
function updateElementVariablesFile(config, spinner, cb) {
...
function update_theme_variables(cb){
  var spinner = ora('begin update file...').start()
  config = readConfigJson(spinner)
  if(config){
    updateElementVariablesFile(config, spinner, cb)
  }else{
    cb()
  }
}
exports.update_theme_variables = update_theme_variables